// GLSL 着色器 —— 移植自 Mineradio (GPL-3.0)，逐字保留其粒子视觉。
//   来源: XxHuberrr/Mineradio public/index.html（顶点 5858–6329 / 片元 5332–5357 / bloom 5364–5398）
// 只把音频 uniform 的喂法换成 Nobi 的 bands/peak（见 particles.js），着色器数学原样不动。
// 全局对象 window.WallpaperShaders。
...
void main(){
  ...
  // bloom 顶点着色器 = 主 vs 的两处替换（加 uBloomSize、点大小再乘一下）。
  const bloomVs = vs
    .replace(
      'uniform float uMouseActive, uPixel, uColorMixT, uLoading;',
  ...
  window.WallpaperShaders = { vs, fs, bloomVs, bloomFs };
})();
