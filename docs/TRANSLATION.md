...
- `src-tauri/resources/ecdict.db` = **精简版 ~3.7 万常用词、4.9MB**，打包进安装包（`tauri.conf.json` bundle.resources），**已提交入 git**（`.gitignore` 对它开了例外 `!src-tauri/resources/ecdict.db`，免得换机器/github 被挡时无法重建）。clone 即有，无需手动补。
- 精简口径：考试标签(中考/高考/四六级/考研/托福) ∪ 牛津3000 ∪ 柯林斯2★ ∪ 词频(frq/bnc)前 3 万。生僻词查不到时自动回落在线。
- 重建/换全量：`node --experimental-sqlite scripts/build-ecdict-mini.mjs <ecdict.csv> [out.db]`（`ecdict.csv` 在 ECDICT 仓库根，77 万词 65MB；改 `FRQ_MAX` 调大小）。要全量就直接放完整 `stardict.db`（schema 一致）。
- 代码：`translation.rs::ecdict_lookup`/`parse_ecdict_translation`（按真换行拆义项、行首词性）/`is_word_lookup`。`scripts/make-ecdict-fixture.mjs` 是早期 12 词占位生成器，已被 build-ecdict-mini 取代。
...
1. ~~换真 ECDICT 词典~~ ✅ 已是 3.7 万词精简版、入库、随包。（如需全量 77 万词，按上节重建）
2. `tauri build` 出包后**在干净机器验证**：装包（基线小，含 5MB 词典、不含 NMT 模型）→ 离线查词应直接可用 → 应用内点「下载离线翻译包」从 HF 拉成功 → **断网后离线翻整句可用**；并确认 onnxruntime 原生库真进了安装包。
