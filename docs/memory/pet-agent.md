...
**v0.5.3**：① 桌宠自适应去透明边——选皮肤时 canvas 扫 idle 行几帧算非透明 bbox(按 url 缓存)，PetSprite 按 bbox 缩放居中填满 size 方框(window≈角色)，气泡贴窗口边/贴屏幕边的空白随之消失；idle bbox 给所有行共用+2px 余量(极端帧可能轻微裁，需要再扫多行)。② 说话间隔可调(`nobi-winky-speakmin-v1`，分钟，风格页；定时器首句 25–45s 后用 speakMin±jitter)。③ 自言自语台词 25→43 句(通用18+时段5×5)。④ chatOnce 失败记一条 err 到 log(chatterErrRef 只记首次)，排查"为啥没用 API"。纯前端，无 Rust 改动。
